 ...
 */
import { _ as jodaInternal, DateTimeFormatterBuilder, ChronoField, TextStyle } from '@js-joda/core';
...
export default class LocalizedOffsetPrinterParser {
    ...
    print(context: PrintContext, buf: TextBuffer): boolean {
        ...
        if (totalSecs !== 0) {
            const absHours: number = Math.abs(MathUtil.intMod(MathUtil.intDiv(totalSecs, 3600), 100)); // anything larger than 99 silently dropped
            const absMinutes: number = Math.abs(MathUtil.intMod(MathUtil.intDiv(totalSecs, 60), 60));
            const absSeconds: number = Math.abs(MathUtil.intMod(totalSecs, 60));
    ...
    parse(context: ParseContext, text: string, position: number): number {
        ...
        if (position === end || text.charAt(position) !== ':') {
            const offset: number = negative * 3600 * hour;
        ...
        if (position === end || text.charAt(position) !== ':') {
            const offset: number = negative * (3600 * hour + 60 * min);
            return context.setParsedField(ChronoField.OFFSET_SECONDS, offset, position, position);
        }
        position++;
        // second
        ...
        const offset: number = negative * (3600 * hour + 60 * min + sec);
        return context.setParsedField(ChronoField.OFFSET_SECONDS, offset, position, position);
    }
    toString(): string {
