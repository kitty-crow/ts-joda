...
import type { Temporal } from "./temporal/Temporal.ts";
...
import type { ValueRange } from "./temporal/ValueRange.ts";
/*
...
export class ZonedDateTime extends ChronoZonedDateTime {
     ...
     */
    static of(dateTimeOrYear: LocalDateTime | LocalDate | number, zoneOrTimeOrMonth: ZoneId | LocalTime | number, zoneOrDay?: ZoneId | number, hour?: number, minute?: number, second?: number, nanoOfSecond?: number, finalZone?: ZoneId): ZonedDateTime {
        if (dateTimeOrYear instanceof LocalDateTime && zoneOrTimeOrMonth instanceof ZoneId) {
            return ZonedDateTime.of2(dateTimeOrYear, zoneOrTimeOrMonth);
        }
        if (dateTimeOrYear instanceof LocalDate && zoneOrTimeOrMonth instanceof LocalTime && zoneOrDay instanceof ZoneId) {
            return ZonedDateTime.of3(dateTimeOrYear, zoneOrTimeOrMonth, zoneOrDay);
        }
        return ZonedDateTime.of8(
            dateTimeOrYear as number,
            zoneOrTimeOrMonth as number,
            requireNonNull(zoneOrDay as number | undefined, 'dayOfMonth'),
            requireNonNull(hour, 'hour'),
            requireNonNull(minute, 'minute'),
            requireNonNull(second, 'second'),
            requireNonNull(nanoOfSecond, 'nanoOfSecond'),
            requireNonNull(finalZone, 'zone'),
        );
    }
    /**
     ...
     */
    static of2(localDateTime: LocalDateTime, zone: ZoneId): ZonedDateTime {
     ...
     */
    static of8(year: number, month: number, dayOfMonth: number, hour: number, minute: number, second: number, nanoOfSecond: number, zone: ZoneId): ZonedDateTime {
    ...
    static ofLocal(localDateTime: LocalDateTime, zone: ZoneId, preferredOffset?: ZoneOffset | null): ZonedDateTime {
        ...
        if (validOffsets.length === 1) {
            ...
            const trans = requireNonNull(rules.transition(localDateTime), 'transition');
     ...
     */
    static ofInstant(instantOrDateTime: Instant | LocalDateTime, zoneOrOffset: ZoneId | ZoneOffset, finalZone?: ZoneId): ZonedDateTime {
        if (instantOrDateTime instanceof Instant) {
            return ZonedDateTime.ofInstant2(instantOrDateTime, zoneOrOffset);
        }
        return ZonedDateTime.ofInstant3(
            instantOrDateTime,
            zoneOrOffset as ZoneOffset,
            requireNonNull(finalZone, 'zone'),
        );
    }
    /**
     ...
     * @return {ZonedDateTime} the zoned date-time, not null
     */
    static ofInstant3(localDateTime: LocalDateTime, offset: ZoneOffset, zone: ZoneId): ZonedDateTime {
        requireNonNull(localDateTime, 'localDateTime');
     ...
     * @throws DateTimeException if the result exceeds the supported range
     */
    static _create(epochSecond: number, nanoOfSecond: number, zone: ZoneId): ZonedDateTime {
        const rules = zone.rules();
     ...
     * @return {ZonedDateTime} the zoned date-time, not null
     */
    static ofLenient(localDateTime: LocalDateTime, offset: ZoneOffset, zone: ZoneId): ZonedDateTime {
        requireNonNull(localDateTime, 'localDateTime');
        requireNonNull(offset, 'offset');
        requireNonNull(zone, 'zone');
    ...
    static _from(temporal: TemporalAccessor, zone: ZoneId): ZonedDateTime | undefined {
        try {
    ...
    static __from(temporal: TemporalAccessor, zone: ZoneId): ZonedDateTime {
     ...
     */
    static parse(text: string, formatter: DateTimeFormatter  = DateTimeFormatter.ISO_ZONED_DATE_TIME): ZonedDateTime {
     ...
     * @return {ZonedDateTime} the zoned date-time, not null
     */
    _resolveLocal(newDateTime: LocalDateTime): ZonedDateTime {
     ...
     * @return {ZonedDateTime} the zoned date-time, not null
     */
    _resolveInstant(newDateTime: LocalDateTime): ZonedDateTime {
     ...
     * @return {ZonedDateTime} the zoned date-time, not null
     */
    _resolveOffset(offset: ZoneOffset): ZonedDateTime {
     ...
     * @throws ArithmeticException if numeric overflow occurs
     */
    _withAdjuster(adjuster: TemporalAdjuster): ZonedDateTime {
        // optimizations
        ...
        return super._withAdjuster(adjuster) as ZonedDateTime;
    }
    /**
     ...
     * @throws ArithmeticException if numeric overflow occurs
     */
    _withField(field: TemporalField, newValue: number): ZonedDateTime {
        if (field instanceof ChronoField) {
            switch (field) {
        ...
        return field.adjustInto(this, newValue) as ZonedDateTime;
    }
    //-----------------------------------------------------------------------
    /**
     ...
     */
    _plusUnit(amountToAdd: number, unit: TemporalUnit): ZonedDateTime {
     ...
     * @throws DateTimeException if the unit cannot be added to this type
     */
    _minusUnit(amountToSubtract: number, unit: TemporalUnit): ZonedDateTime {
...
export function _init(): void {
    ZonedDateTime.FROM = createTemporalQuery('ZonedDateTime.FROM', (temporal: TemporalAccessor): ZonedDateTime => {
