 ...
 */
import { MAX_SAFE_INTEGER, MIN_SAFE_INTEGER } from "../MathUtil.ts";
...
export class ChronoField extends TemporalField {
    ...
    _name!: string;
    readonly _baseUnit: TemporalUnit;
    readonly _rangeUnit: TemporalUnit;
    _range!: ValueRange;
    /**
     ...
     * @private
     */
    static byName(fieldName: string): ChronoField | null {
        for (const value of Object.values(ChronoField)) {
            if (value instanceof ChronoField && value.name() === fieldName) {
                return value;
            }
        }
        return null;
     ...
     */
    isDateBased(): boolean {
        const dateBased = this === ChronoField.DAY_OF_WEEK ||
     ...
     */
    isTimeBased(): boolean {
        const timeBased = this === ChronoField.NANO_OF_SECOND ||
