...
export class Instant extends Temporal {
     ...
     */
    static now(clock: Clock  = Clock.systemUTC()): Instant {
     ...
     */
    static ofEpochSecond(epochSecond: number, nanoAdjustment: number  = 0): Instant {
     ...
     * @private
     */
    static _create(seconds: number, nanoOfSecond: number): Instant {
     ...
     * @private
     */
    static _validate(seconds: number, nanoOfSecond: number): void {
     ...
     * @private
     */
    constructor(seconds: number, nanoOfSecond: number) {
        super();
     ...
     * @throws ArithmeticException if numeric overflow occurs
     */
    _withField(field: TemporalField, newValue: number): Instant {
     ...
     */
    _plusUnit(amountToAdd: number, unit: TemporalUnit): Instant {
     ...
     * @throws DateTimeException if the result exceeds the maximum or minimum instant
     */
    _plus(secondsToAdd: number | 0, nanosToAdd: number | 0): Instant {
     ...
     * @throws ArithmeticException
     */
    _minusUnit(amountToSubtract: number, unit: TemporalUnit): Instant {
     ...
     * @private
     */
    _microsUntil(end: Instant): number {
     ...
     * @private
     */
    _nanosUntil(end: Instant): number {
        const secsDiff = MathUtil.safeSubtract(end.epochSecond(), this.epochSecond());
     ...
     * @private
     */
    _secondsUntil(end: Instant): number {
...
export function _init(): void {
    ...
    Instant.FROM = createTemporalQuery('Instant.FROM', (temporal: TemporalAccessor): Instant => {
