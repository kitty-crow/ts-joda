...
import { DateTimeException, UnsupportedTemporalTypeException } from "./errors.ts";
...
export class LocalDateTime extends ChronoLocalDateTime {
     ...
     */
    static _now(clock: Clock): LocalDateTime {
     ...
     *
     */
    static _ofEpochMillis(epochMilli: number, offset: ZoneOffset): LocalDateTime {
        ...
        const time = LocalTime.ofSecondOfDay(secsOfDay, nanoOfSecond as number);
     ...
     */
    static of(dateOrYear: LocalDate | number, timeOrMonth: LocalTime | Month | number, dayOfMonth?: number, hour?: number, minute?: number, second?: number, nanoSecond?: number): LocalDateTime {
        if (arguments.length <= 2) {
            return LocalDateTime.ofDateAndTime(dateOrYear as LocalDate, timeOrMonth as LocalTime);
        }
        return LocalDateTime.ofNumbers(dateOrYear as number, timeOrMonth as Month | number, dayOfMonth as number, hour, minute, second, nanoSecond);
    }
    /**
     ...
     */
    static ofNumbers(year: number, month: Month | number, dayOfMonth: number, hour = 0, minute = 0, second = 0, nanoOfSecond = 0): LocalDateTime {
     ...
     */
    static ofInstant(instant: Instant, zone: ZoneId  = ZoneId.systemDefault()): LocalDateTime {
     ...
     * @throws {DateTimeException} if the result exceeds the supported range
     */
    static ofEpochSecond(epochSecond: number = 0, nanoOfSecond: (number) | (ZoneOffset) = 0, offset?: ZoneOffset): LocalDateTime {
        if (arguments.length === 2 && nanoOfSecond instanceof ZoneOffset) {
            offset = nanoOfSecond;
            nanoOfSecond = 0;
        }
        const resolvedOffset = requireNonNull(offset, 'offset');
        const localSecond = epochSecond + resolvedOffset.totalSeconds(); // overflow caught later
        const localEpochDay = MathUtil.floorDiv(localSecond, LocalTime.SECONDS_PER_DAY);
        const secsOfDay = MathUtil.floorMod(localSecond, LocalTime.SECONDS_PER_DAY);
        const date = LocalDate.ofEpochDay(localEpochDay);
        const time = LocalTime.ofSecondOfDay(secsOfDay, nanoOfSecond as number);
     ...
     */
    static parse(text: string, formatter: DateTimeFormatter  = DateTimeFormatter.ISO_LOCAL_DATE_TIME): LocalDateTime {
     ...
     * @return {LocalDateTime} the date-time, not null
     */
    _withDateTime(newDate: LocalDate, newTime: LocalTime): LocalDateTime {
     ...
     * @throws {ArithmeticException} if numeric overflow occurs
     */
    _withAdjuster(adjuster: TemporalAdjuster): LocalDateTime {
        ...
        return super._withAdjuster(adjuster) as LocalDateTime;
    }
    /**
     ...
     * @throws {ArithmeticException} if numeric overflow occurs
     */
    _withField(field: TemporalField, newValue: number): LocalDateTime {
        requireNonNull(field, 'field');
        if (field instanceof ChronoField) {
     ...
     */
    _plusUnit(amountToAdd: number, unit: TemporalUnit): LocalDateTime {
     ...
     * @throws {DateTimeException} if the unit cannot be added to this type
     */
    _minusUnit(amountToSubtract: number, unit: TemporalUnit): LocalDateTime {
        requireNonNull(unit, 'unit');
     ...
     */
    _plusWithOverflow(newDate: LocalDate, hours: number | 0, minutes: number | 0, seconds: number | 0, nanos: number | 0, sign: number | 1 | -1): LocalDateTime {
     ...
     */
    override compareTo(other: ChronoLocalDateTime): number {
        requireNonNull(other, 'other');
        if (other instanceof LocalDateTime) {
            return this._compareTo0(other);
        }
        let cmp = MathUtil.compareNumbers(this._date.toEpochDay(), other.toLocalDate().toEpochDay());
        if (cmp === 0) {
     ...
     * @private
     */
    _compareTo0(other: LocalDateTime): number {
        let cmp = MathUtil.compareNumbers(this._date.toEpochDay(), other.toLocalDate().toEpochDay());
        if (cmp === 0) {
            cmp = this._time.compareTo(other.toLocalTime());
        }
...
export function _init(): void {
    ...
    LocalDateTime.FROM = createTemporalQuery('LocalDateTime.FROM', (temporal: TemporalAccessor): LocalDateTime => {
