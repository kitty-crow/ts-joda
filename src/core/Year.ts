...
import { TemporalQuery, createTemporalQuery } from "./temporal/TemporalQuery.ts";
...
export class Year extends Temporal {
     ...
     */
    static nowZoneId(zone: ZoneId): Year {
     ...
     */
    static nowClock(clock: Clock): Year {
     ...
     */
    static parseText(text: string): Year {
     ...
     * @throws DateTimeParseException if the text cannot be parsed
     */
    static parseTextFormatter(text: string, formatter: DateTimeFormatter = PARSER): Year {
        requireNonNull(text, 'text');
     ...
     * @throws ArithmeticException if numeric overflow occurs
     */
    _withField(field: TemporalField, newValue: number): Year {
        requireNonNull(field, 'field');
     ...
     */
    atMonthMonth(month: Month): YearMonth {
     ...
     */
    atMonthNumber(month: number | (number | Month)): YearMonth {
        requireNonNull(month, 'month');
        return YearMonth.of(this._year, month);
    }
    /**
    ...
    query<R>(query: TemporalQuery<R>): R | null {
        ...
        if (query === TemporalQueries.chronology()) {
            ...
            return ChronoUnit.YEARS as unknown as R;
        }
...
export function _init(): void {
    ...
    PARSER = new DateTimeFormatterBuilder()
        ...
        .toFormatter();
    Year.FROM = createTemporalQuery('Year.FROM', (temporal: TemporalAccessor): Year => {
