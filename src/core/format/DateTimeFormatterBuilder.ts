...
import type { DateTimePrinterParser } from "./parser/DateTimePrinterParser.ts";
 ...
 */
import { assert, requireNonNull, requireInstance } from "../assert.ts";
import { IllegalArgumentException, IllegalStateException } from "../errors.ts";
...
import { DecimalStyle } from "./DecimalStyle.ts";
import { SignStyle } from "./SignStyle.ts";
import { TextStyle } from "./TextStyle.ts";
import { ResolverStyle } from "./ResolverStyle.ts";
import { CharLiteralPrinterParser } from "./parser/CharLiteralPrinterParser.ts";
import { CompositePrinterParser } from "./parser/CompositePrinterParser.ts";
import { FractionPrinterParser } from "./parser/FractionPrinterParser.ts";
import { NumberPrinterParser, ReducedPrinterParser } from "./parser/NumberPrinterParser.ts";
import { OffsetIdPrinterParser } from "./parser/OffsetIdPrinterParser.ts";
import { PadPrinterParserDecorator } from "./parser/PadPrinterParserDecorator.ts";
import { SettingsParser } from "./parser/SettingsParser.ts";
import { StringLiteralPrinterParser } from "./parser/StringLiteralPrinterParser.ts";
import { ZoneIdPrinterParser } from "./parser/ZoneIdPrinterParser.ts";
const MAX_WIDTH = 15; // can't parse all numbers with more then 15 digits in javascript
export class DateTimeFormatterBuilder {
    static CompositePrinterParser: typeof CompositePrinterParser;
    static PadPrinterParserDecorator: typeof PadPrinterParserDecorator;
    static SettingsParser: typeof SettingsParser;
    static CharLiteralPrinterParser: typeof CharLiteralPrinterParser;
    static StringLiteralPrinterParser: typeof StringLiteralPrinterParser;
    static NumberPrinterParser: typeof NumberPrinterParser;
    static ReducedPrinterParser: typeof ReducedPrinterParser;
    static FractionPrinterParser: typeof FractionPrinterParser;
    static OffsetIdPrinterParser: typeof OffsetIdPrinterParser;
    static ZoneIdPrinterParser: typeof ZoneIdPrinterParser;

    _active: DateTimeFormatterBuilder;
    _parent: DateTimeFormatterBuilder | null;
    _printerParsers: DateTimePrinterParser[];
    _optional: boolean;
    _padNextWidth: number;
    _padNextChar: string | null;
    _valueParserIndex: number;
    /**
     ...
     */
    static _of(parent: DateTimeFormatterBuilder, optional: boolean): DateTimeFormatterBuilder {
    ...
    appendValue(field: TemporalField, width?: number, maxWidth?: number, signStyle?: SignStyle): DateTimeFormatterBuilder {
        if (width === undefined) {
            return this._appendValue1(field);
        }
        if (maxWidth === undefined) {
            return this._appendValue2(field, width);
        }
        if (signStyle === undefined) {
            throw new IllegalArgumentException('SignStyle is required with minimum and maximum widths');
        }
        return this._appendValue4(field, width, maxWidth, signStyle);
    }
    /**
     ...
     * @return {DateTimeFormatterBuilder} this, for chaining, not null
     */
    _appendValue1(field: TemporalField): DateTimeFormatterBuilder {
        requireNonNull(field);
     ...
     */
    _appendValue2(field: TemporalField, width: number): this {
        requireNonNull(field);
     ...
     */
    _appendValue4(field: TemporalField, minWidth: number, maxWidth: number, signStyle: SignStyle): DateTimeFormatterBuilder {
        requireNonNull(field);
    ...
    appendValueReduced(field: TemporalField, width: number, maxWidth: number, base: ChronoLocalDate | number): DateTimeFormatterBuilder {
        return typeof base === 'number'
            ? this._appendValueReducedFieldWidthMaxWidthBaseValue(field, width, maxWidth, base)
            : this._appendValueReducedFieldWidthMaxWidthBaseDate(field, width, maxWidth, base);
    }
    /**
     ...
     */
    _appendValueReducedFieldWidthMaxWidthBaseValue(field: TemporalField, width: number, maxWidth: number, baseValue: number): DateTimeFormatterBuilder {
     ...
     * @throws IllegalArgumentException if the width or base value is invalid
     */
    _appendValueReducedFieldWidthMaxWidthBaseDate(field: TemporalField, width: number, maxWidth: number, baseDate: ChronoLocalDate): DateTimeFormatterBuilder {
        requireNonNull(field, 'field');
     ...
     * @return {DateTimeFormatterBuilder} this, for chaining, not null
     */
    _appendValuePrinterParser(pp: NumberPrinterParser | ReducedPrinterParser): DateTimeFormatterBuilder {
        assert(pp != null);
        if (this._active._valueParserIndex >= 0 &&
            ...
            let basePP = this._active._printerParsers[activeValueParser] as NumberPrinterParser;
     ...
     * @return {DateTimeFormatterBuilder} this, for chaining, not null
     */
    appendInstant(fractionalDigits: number  = -2): DateTimeFormatterBuilder {
    ...
    appendZoneText(_style: TextStyle): DateTimeFormatterBuilder {
        throw new IllegalArgumentException('Pattern using (localized) text not implemented, use @js-joda/locale plugin!');
    }
    appendText(
        _field: TemporalField,
        _styleOrMap: TextStyle | Readonly<Record<string | number, string>> = TextStyle.FULL,
    ): DateTimeFormatterBuilder {
        throw new IllegalArgumentException('Pattern using (localized) text not implemented, use @js-joda/locale plugin!');
    }
    appendLocalizedOffset(_style: TextStyle): DateTimeFormatterBuilder {
        throw new IllegalArgumentException('Pattern using (localized) text not implemented, use @js-joda/locale plugin!');
    }
    appendWeekField(_letter: string, _count: number): DateTimeFormatterBuilder {
        throw new IllegalArgumentException('Pattern using (localized) text not implemented, use @js-joda/locale plugin!');
    }
    //-----------------------------------------------------------------------
    _parsePattern(pattern: string): void {
        /** Map of letters to fields. */
        const FIELD_MAP: Record<string, TemporalField> = {
            'G': ChronoField.ERA,
    ...
    _parseField(cur: string, count: number, field: TemporalField): void {
        switch (cur) {
            case 'u':
            case 'y':
                if (count === 2) {
    ...
    padNext(padWidth: number, padChar?: string): DateTimeFormatterBuilder {
        return padChar === undefined
            ? this._padNext1(padWidth)
            : this._padNext2(padWidth, padChar);
    }
    /**
     ...
     */
    _padNext1(padWidth: number): DateTimeFormatterBuilder {
     ...
     * @throws IllegalArgumentException if pad width is too small
     */
    _padNext2(padWidth: number, padChar: string | " "): DateTimeFormatterBuilder {
        if (padWidth < 1) {
     ...
     */
    _appendInternal(pp: DateTimePrinterParser): number {
        assert(pp != null);
        if (this._active._padNextWidth > 0) {
            ...
            this._active._padNextChar = null;
        }
     ...
     * @return the index into the active parsers list
     */
    _appendInternalPrinterParser(pp: DateTimePrinterParser): number {
        assert(pp != null);
        if (this._active._padNextWidth > 0) {
            if (pp != null) {
                pp = new PadPrinterParserDecorator(pp, this._active._padNextWidth, this._active._padNextChar ?? ' ');
     ...
     */
    toFormatter(resolverStyle: ResolverStyle  = ResolverStyle.SMART): DateTimeFormatter {
 ...
 */
class InstantPrinterParser implements DateTimePrinterParser {
    readonly fractionalDigits: number;
    constructor(fractionalDigits: number) {
    ...
    parse(context: DateTimeParseContext, text: string, position: number): number {
        ...
        if (yearParsed === undefined || month === undefined || day === undefined || hour === undefined || min === undefined) {
            return ~position;
        }
 ...
 * @private
 */
class DefaultingParser implements DateTimePrinterParser {
     ...
     * @param {number} value
     */
    constructor(field: TemporalField, value: number) {
        this._field = field;
        this._value = value;
    }
    /**
...
export function _init(): void {
    ReducedPrinterParser.BASE_DATE = LocalDate.of(2000, 1, 1) as unknown as ChronoLocalDate;
