...
import type { CompositePrinterParser } from "./parser/CompositePrinterParser.ts";
import type { Parsed } from "./DateTimeParseContext.ts";
/**
...
import { DateTimeParseException, NullPointerException } from "../errors.ts";
import { Period } from "../Period.ts";
import { ParsePosition } from "./ParsePosition.ts";
import { DateTimeBuilder } from "./DateTimeBuilder.ts";
import { DateTimeParseContext } from "./DateTimeParseContext.ts";
import { DateTimePrintContext } from "./DateTimePrintContext.ts";
import { DateTimeFormatterBuilder } from "./DateTimeFormatterBuilder.ts";
...
export class DateTimeFormatter {
    ...
    static PARSED_EXCESS_DAYS: TemporalQuery<Period>;
    static PARSED_LEAP_SECOND: TemporalQuery<boolean>;
    readonly _printerParser: CompositePrinterParser;
    readonly _locale: unknown;
    readonly _decimalStyle: DecimalStyle;
    readonly _resolverStyle: ResolverStyle;
    readonly _resolverFields: TemporalField[] | null;
    readonly _chrono: IsoChronology | null;
    readonly _zone: ZoneId | null;
    //-----------------------------------------------------------------------
    /**
    ...
    constructor(
        ...
        resolverFields: TemporalField[] | null,
        chrono: IsoChronology | null,
        zone: ZoneId | null,
    ) {
     ...
     */
    withChronology(chrono: IsoChronology): DateTimeFormatter {
     ...
     */
    withLocale(_locale: unknown): DateTimeFormatter {
        return this;
    }
    /**
     ...
     * @throws DateTimeException if an error occurs during formatting
     */
    _formatTo(temporal: TemporalAccessor, appendable: StringBuilder): void {
     ...
     */
    parse<T>(text: string, type?: TemporalQuery<T>): TemporalAccessor | T {
        if (type === undefined) {
     ...
     */
    parse1(text: string): DateTimeBuilder {
 ...
 * @return the parsed temporal object, not null
     * @throws DateTimeParseException if unable to parse the requested result
     */
    parse2<T>(text: string, type: TemporalQuery<T>): T {
        requireNonNull(text, 'text');
    ...
    _createError(text: string, ex: unknown): DateTimeParseException {
        let abbr = '';
        if (text.length > 64) {
        ...
        return new DateTimeParseException(`Text '${abbr}' could not be parsed: ${message}`, text, 0, ex);
    }
    /**
     ...
     */
    _parseToBuilder(text: string, position: ParsePosition | null): DateTimeBuilder {
     ...
     */
    parseUnresolved(text: string, position: ParsePosition): TemporalAccessor | null {
        return this._parseUnresolved0(text, position);
    }
    _parseUnresolved0(text: string, position: ParsePosition): Parsed | null {
     ...
     */
    _toPrinterParser(optional: boolean): CompositePrinterParser {
...
export function _init(): void {
    ...
    DateTimeFormatter.PARSED_EXCESS_DAYS = createTemporalQuery<Period>('PARSED_EXCESS_DAYS', (temporal: TemporalAccessor): Period => {
        if (temporal instanceof DateTimeBuilder) {
            return temporal.excessDays ?? Period.ZERO;
        }
        else {
            return Period.ZERO;
        }
    });
    DateTimeFormatter.PARSED_LEAP_SECOND = createTemporalQuery<boolean>('PARSED_LEAP_SECOND', (temporal: TemporalAccessor): boolean => {
        if (temporal instanceof DateTimeBuilder) {
