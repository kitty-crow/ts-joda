 ...
 */
import { requireNonNull, requireInstance } from "./assert.ts";
import { ArithmeticException, DateTimeParseException, UnsupportedTemporalTypeException } from "./errors.ts";
import { MathUtil, MAX_SAFE_INTEGER, MIN_SAFE_INTEGER } from "./MathUtil.ts";
...
export class Duration extends TemporalAmount /*implements TemporalAmount, Comparable<Duration>, Serializable */ {
     ...
     */
    static ofSeconds(seconds: number, nanoAdjustment: number  = 0): Duration {
    ...
    static from(amount: TemporalAmount): Duration {
        ...
        amount.units().forEach((unit: TemporalUnit): void => {
    ...
    static _parseNumber(text: string, parsed: string | undefined, multiplier: number, errorText: "days" | "hours" | "minutes" | "seconds"): number {
        // regex limits to [-+]?[0-9]+
        if (parsed == null) {
            return 0;
        }
    ...
    static _parseFraction(text: string, parsed: string | undefined, negate: 1 | -1): number {
     ...
     * @return {Duration}
     */
    static _create(seconds?: number, nanos?: number): Duration;
    static _create(negate: boolean, daysAsSecs: number, hoursAsSecs: number, minsAsSecs: number, secs: number, nanos: number): Duration;
    static _create(...args: unknown[]): Duration {
        if (args.length <= 2) {
            return Duration._createSecondsNanos(args[0] as number | undefined, args[1] as number | undefined);
        }
        const [negate, daysAsSecs, hoursAsSecs, minsAsSecs, secs, nanos] = args as [boolean, number, number, number, number, number];
        return Duration._createNegateDaysHoursMinutesSecondsNanos(negate, daysAsSecs, hoursAsSecs, minsAsSecs, secs, nanos);
    }
    static _createNegateDaysHoursMinutesSecondsNanos(negate: boolean, daysAsSecs: number, hoursAsSecs: number, minsAsSecs: number, secs: number, nanos: number): Duration {
     ...
     */
    static _createSecondsNanos(seconds = 0, nanoAdjustment = 0): Duration {
    ...
    plus(durationOrNumber: number | Duration, unitOrNumber?: TemporalUnit | number): Duration {
        if (arguments.length === 1) {
            return this.plusDuration(durationOrNumber as Duration);
        }
        if (unitOrNumber instanceof TemporalUnit) {
            return this.plusAmountUnit(durationOrNumber as number, unitOrNumber);
        }
        return this.plusSecondsNanos(durationOrNumber as number, unitOrNumber as number);
    }
    /**
     * Returns a copy of this duration with the specified duration added.
     *
    ...
    minus(durationOrNumber: number | Duration, unit?: TemporalUnit): Duration {
        if (arguments.length === 1) {
            return this.minusDuration(durationOrNumber as Duration);
        }
        return this.minusAmountUnit(durationOrNumber as number, unit as TemporalUnit);
    }
    /**
    ...
    minusDuration(duration: Duration): Duration {
        ...
        if (secsToSubtract === MIN_SAFE_INTEGER) {
            return this.plusSecondsNanos(MAX_SAFE_INTEGER, -nanosToSubtract);
        }
        return this.plusSecondsNanos(-secsToSubtract, -nanosToSubtract);
    }
    /**
     * Returns a copy of this duration with the specified duration subtracted.
     *
    ...
    addTo<T extends Temporal>(temporal: T): T {
        ...
        if (this._seconds !== 0) {
            temporal = temporal.plus(this._seconds, ChronoUnit.SECONDS) as T;
        }
        if (this._nanos !== 0) {
            temporal = temporal.plus(this._nanos, ChronoUnit.NANOS) as T;
        }
        return temporal;
    }
    /**
    ...
    subtractFrom<T extends Temporal>(temporal: T): T {
        requireNonNull(temporal, 'temporal');
        if (this._seconds !== 0) {
            temporal = temporal.minus(this._seconds, ChronoUnit.SECONDS) as T;
        }
        if (this._nanos !== 0) {
            temporal = temporal.minus(this._nanos, ChronoUnit.NANOS) as T;
        }
        return temporal;
    }
    //-----------------------------------------------------------------------
