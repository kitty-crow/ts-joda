...
import { DateTimeException, UnsupportedTemporalTypeException, NullPointerException, IllegalArgumentException } from "./errors.ts";
...
export class LocalDate extends ChronoLocalDate {
     ...
     */
    static ofInstant(instant: Instant, zone: ZoneId  = ZoneId.systemDefault()): LocalDate {
     ...
     */
    static ofEpochDay(epochDay: number = 0): LocalDate {
    ...
    static from(temporal: TemporalAccessor): LocalDate {
        ...
        const date = temporal.query(TemporalQueries.localDate()) as LocalDate | null;
        if (date == null) {
     ...
     */
    static parse(text: string, formatter: DateTimeFormatter  = DateTimeFormatter.ISO_LOCAL_DATE): LocalDate {
     ...
     */
    static _resolvePreviousValid(year: number, month: number, day: number): LocalDate {
        switch (month) {
            case 2:
     ...
     * @private
     */
    constructor(year: number, month: number | Month, dayOfMonth: number) {
        super();
     ...
     * @private
     */
    static _validate(year: number, month: number, dayOfMonth: number): void {
        let dom;
        ChronoField.YEAR.checkValidValue(year);
     ...
     */
    isSupported(field: TemporalField | TemporalUnit): boolean {
     ...
     */
    override chronology(): IsoChronology {
     ...
     * @throws {ArithmeticException} if numeric overflow occurs
     */
    _withAdjuster(adjuster: TemporalAdjuster): LocalDate {
        requireNonNull(adjuster, 'adjuster');
        // optimizations
        if (adjuster instanceof LocalDate) {
            return adjuster;
        }
        return super._withAdjuster(adjuster) as LocalDate;
    }
    /**
     ...
     * @throws {ArithmeticException} if numeric overflow occurs
     */
    _withField(field: TemporalField, newValue: number): LocalDate {
     ...
     */
    _plusUnit(amountToAdd: number, unit: TemporalUnit): LocalDate {
     ...
     * @throws {DateTimeException} if the unit cannot be added to this type
     */
    _minusUnit(amountToSubtract: number, unit: TemporalUnit): LocalDate {
     ...
     */
    until(p1: (TemporalAccessor) | (Temporal), p2?: TemporalUnit): (Period) | (number) {
        if (arguments.length < 2) {
            return this.until1(p1);
        }
        else {
            return this.until2(p1, p2 as TemporalUnit);
        }
    }
    /**
     ...
     * @protected
     */
    daysUntil(end: LocalDate): number {
     ...
     * @private
     */
    _monthsUntil(end: LocalDate): number {
     ...
     */
    until1(endDate: TemporalAccessor): Period {
     ...
     */
    atTime(hourOrTime: number | LocalTime | OffsetTime, minute?: number, second?: number, nanoOfSecond?: number): LocalDateTime | OffsetDateTime {
        if (arguments.length === 1) {
            return this.atTime1(hourOrTime as LocalTime | OffsetTime);
        }
        return this.atTime4(hourOrTime as number, minute as number, second, nanoOfSecond);
    }
    /**
     ...
     */
    atTime1(time: LocalTime | OffsetTime): OffsetDateTime | LocalDateTime {
     ...
     * @throws {DateTimeException} if the value of any field is out of range
     */
    atTime4(hour: number, minute: number, second = 0, nanoOfSecond = 0): LocalDateTime {
     ...
     */
    _atTimeOffsetTime(time: OffsetTime): OffsetDateTime {
     ...
     */
    atStartOfDay(zone?: ZoneId): (LocalDateTime) | (ZonedDateTime) {
        if (zone != null) {
     ...
     */
    _atStartOfDayWithZone(zone: ZoneId): ZonedDateTime {
     ...
     * @private
     */
    _compareTo0(otherDate: LocalDate): number {
...
export function _init(): void {
    ...
    LocalDate.FROM = createTemporalQuery('LocalDate.FROM', (temporal: TemporalAccessor): LocalDate => {
