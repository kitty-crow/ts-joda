...
export class YearMonth extends Temporal {
    ...
    static now(zoneIdOrClock?: ZoneId | Clock): YearMonth {
        if (arguments.length === 0) {
            ...
            return YearMonth.nowClock(zoneIdOrClock as Clock);
        }
    }
    /**
    ...
    static of(year: number, monthOrNumber: Month | number): YearMonth {
        if (arguments.length === 2 && monthOrNumber instanceof Month) {
            ...
            return YearMonth.ofNumberNumber(year, monthOrNumber as number);
        }
    }
    /**
    ...
    static parse(text: string, formatter?: DateTimeFormatter): YearMonth {
        if (arguments.length === 1) {
            ...
            return YearMonth.parseStringFormatter(text, formatter as DateTimeFormatter);
        }
    }
    /**
    ...
    isSupported(fieldOrUnit: TemporalField | TemporalUnit): boolean {
        if (arguments.length === 1 && fieldOrUnit instanceof TemporalField) {
            ...
            return this.isSupportedUnit(fieldOrUnit as TemporalUnit);
        }
    }
    /**
    ...
    isSupportedUnit(unit: TemporalUnit): boolean {
     ...
     * @throws ArithmeticException if numeric overflow occurs
     */
    _withField(field: TemporalField, newValue: number): YearMonth {
        requireNonNull(field, 'field');
        requireInstance(field, TemporalField, 'field');
        if (field instanceof ChronoField) {
            const f = field;
        ...
        return field.adjustInto(this, newValue) as YearMonth;
    }
    //-----------------------------------------------------------------------
    /**
     ...
     * @throws ArithmeticException if numeric overflow occurs
     */
    _plusUnit(amountToAdd: number, unit: TemporalUnit): YearMonth {
        ...
        return unit.addTo(this, amountToAdd) as YearMonth;
    }
    /**
    ...
    minusMonths(monthsToSubtract: number): YearMonth {
        return (monthsToSubtract === MathUtil.MIN_SAFE_INTEGER ? this.plusMonths(MathUtil.MAX_SAFE_INTEGER).plusMonths(1) : this.plusMonths(-monthsToSubtract));
    }
    //-----------------------------------------------------------------------
    /**
    ...
    query<R>(query: TemporalQuery<R>): R | null {
        ...
        if (query === TemporalQueries.chronology()) {
            ...
            return ChronoUnit.MONTHS as unknown as R;
        }
    ...
    adjustInto<T extends Temporal>(temporal: T): T {
        ...
        return temporal.with(ChronoField.PROLEPTIC_MONTH, this._getProlepticMonth()) as T;
    }
    /**
...
export function _init(): void {
    PARSER = new DateTimeFormatterBuilder()
        ...
        .toFormatter();
    YearMonth.FROM = createTemporalQuery('YearMonth.FROM', (temporal: TemporalAccessor): YearMonth => {
