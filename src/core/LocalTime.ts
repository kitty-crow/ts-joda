...
export class LocalTime extends Temporal /** implements Temporal, TemporalAdjuster */ {
     ...
     */
    static _now(clock: (Clock | undefined) | Clock = Clock.systemDefaultZone()): LocalTime {
     ...
     */
    static ofInstant(instant: Instant, zone: ZoneId  = ZoneId.systemDefault()): LocalTime {
     ...
     */
    static ofSecondOfDay(secondOfDay: number  = 0, nanoOfSecond: number  = 0): LocalTime {
     ...
     */
    static ofNanoOfDay(nanoOfDay: number = 0): LocalTime {
    ...
    static from(temporal: TemporalAccessor): LocalTime {
        ...
        const time = temporal.query(TemporalQueries.localTime()) as LocalTime | null;
        if (time == null) {
     ...
     */
    static parse(text: string, formatter: DateTimeFormatter = DateTimeFormatter.ISO_LOCAL_TIME): LocalTime {
     ...
     * @private
     */
    constructor(hour: number = 0, minute: number = 0, second: number = 0, nanoOfSecond: number = 0) {
        super();
    ...
    static _validate(hour: number, minute: number, second: number, nanoOfSecond: number): void {
     ...
     * @private
     */
    _get0(field: ChronoField): number {
        switch (field) {
     ...
     * @throws {ArithmeticException} if numeric overflow occurs
     */
    _withAdjuster(adjuster: TemporalAdjuster): LocalTime {
        requireNonNull(adjuster, 'adjuster');
        // optimizations
        if (adjuster instanceof LocalTime) {
            return adjuster;
        }
        return super._withAdjuster(adjuster) as LocalTime;
    }
    /**
     ...
     * @throws {ArithmeticException} if numeric overflow occurs
     */
    _withField(field: TemporalField, newValue: number): LocalTime {
        requireNonNull(field, 'field');
     ...
     */
    withHour(hour: number = 0): LocalTime {
        if (this._hour === hour) {
            return this;
        }
     ...
     */
    withMinute(minute: number = 0): LocalTime {
     ...
     */
    withSecond(second: number = 0): LocalTime {
     ...
     */
    withNano(nanoOfSecond: number = 0): LocalTime {
     ...
     */
    _plusUnit(amountToAdd: number, unit: TemporalUnit): LocalTime {
        requireNonNull(unit, 'unit');
     ...
     * @throws {DateTimeException} if the unit cannot be added to this type
     */
    _minusUnit(amountToSubtract: number, unit: TemporalUnit): LocalTime {
        requireNonNull(unit, 'unit');
...
export function _init(): void {
    ...
    LocalTime.FROM = createTemporalQuery('LocalTime.FROM', (temporal: TemporalAccessor): LocalTime => {
