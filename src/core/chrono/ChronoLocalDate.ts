 ...
 */
import { abstractMethodFail, requireNonNull, requireInstance } from "../assert.ts";
...
import { IsoChronology } from "./IsoChronology.ts";
/**
...
export class ChronoLocalDate extends Temporal {
    chronology(): IsoChronology {
        return abstractMethodFail('ChronoLocalDate.chronology');
    }

    toEpochDay(): number {
        return abstractMethodFail('ChronoLocalDate.toEpochDay');
    }
    ...
    query<R>(query: TemporalQuery<R>): R | null {
        if (query === TemporalQueries.chronology()) {
            return this.chronology() as unknown as R;
        }
        else if (query === TemporalQueries.precision()) {
            return ChronoUnit.DAYS as unknown as R;
        }
        else if (query === TemporalQueries.localDate()) {
            return LocalDate.ofEpochDay(this.toEpochDay()) as unknown as R;
        }
