...
export class OffsetTime extends Temporal {
    ...
    static now(clockOrZone?: Clock | ZoneId): OffsetTime {
        if (arguments.length === 0) {
            ...
            return OffsetTime._now(Clock.system(clockOrZone as ZoneId));
        }
    }
    /**
     ...
     */
    static _now(clock: Clock): OffsetTime {
     ...
     * @return {OffsetTime}
     */
    static of(timeOrHour: LocalTime | number, offsetOrMinute: ZoneOffset | number, second?: number, nanoOfSecond?: number, finalOffset?: ZoneOffset): OffsetTime {
        if (timeOrHour instanceof LocalTime && offsetOrMinute instanceof ZoneOffset) {
            return OffsetTime.ofTimeAndOffset(timeOrHour, offsetOrMinute);
        }
        return OffsetTime.ofNumbers(
            timeOrHour as number,
            offsetOrMinute as number,
     ...
     * @return {OffsetTime}
     */
    static ofNumbers(hour: number, minute: number, second: number, nanoOfSecond: number, offset: ZoneOffset): OffsetTime {
     ...
     * @return {OffsetTime}
     */
    static ofTimeAndOffset(time: LocalTime, offset: ZoneOffset): OffsetTime {
        return new OffsetTime(time, offset);
    }
    /**
     ...
     * @return {OffsetTime}
     */
    static parse(text: string, formatter: DateTimeFormatter  = DateTimeFormatter.ISO_OFFSET_TIME): OffsetTime {
     ...
     * @private
     */
    constructor(time: LocalTime, offset: ZoneOffset) {
        super();
    ...
    _minusAmount(amount: TemporalAmount): OffsetTime {
        requireNonNull(amount);
        return amount.subtractFrom(this);
    }
    _minusUnit(amountToSubtract: number, unit: TemporalUnit): OffsetTime {
        return this.plus(-1 * amountToSubtract, unit);
    }
    _plusAmount(amount: TemporalAmount): OffsetTime {
        requireNonNull(amount);
     ...
     * @return {Temporal}
     */
    _plusUnit(amountToAdd: number, unit: TemporalUnit): OffsetTime {
    ...
    query<R>(query: TemporalQuery<R>): R | null {
        ...
        if (query === TemporalQueries.precision()) {
            ...
            return this._time as unknown as R;
        }
    ...
    */
     ...
     * @return {OffsetTime}
     */
    withNano(nano: number): OffsetTime {
    ...
    _withAdjuster(adjuster: TemporalAdjuster): OffsetTime {
    ...
    _withField(field: TemporalField, newValue: number): OffsetTime {
        ...
        return field.adjustInto(this, newValue) as OffsetTime;
    }
    /**
     * @private
     * @param {LocalTime} time
     ...
     * @return {OffsetTime}
     */
    _withLocalTimeOffset(time: LocalTime, offset: ZoneOffset): OffsetTime {
...
export function _init(): void {
    ...
    OffsetTime.FROM = createTemporalQuery('OffsetTime.FROM', (temporal: TemporalAccessor): OffsetTime => {
