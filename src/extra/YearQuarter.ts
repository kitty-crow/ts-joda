...
export class YearQuarter extends Temporal {
    ...
    override _withAdjuster(adjuster: TemporalAdjuster): YearQuarter {
    ...
    override _withField(field: TemporalField, newValue: number): YearQuarter {
        requireNonNull(field, 'field');
        requireInstance(field, TemporalField, 'field');
        if (field === IsoFields.QUARTER_OF_YEAR) {
    ...
    override _plusUnit(amountToAdd: number, unit: TemporalUnit): YearQuarter {
        if (unit === IsoFields.QUARTER_YEARS) {
